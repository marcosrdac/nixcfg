...
let
  cfg = config.services.myNextcloud;  # MODIFIED
...
in {
  ...
  imports = [
    (mkRemovedOptionModule [ "services" "myNextcloud" "config" "adminpass" ] ''
      Please use `services.myNextcloud.config.adminpassFile' instead!
    '')
    (mkRemovedOptionModule [ "services" "myNextcloud" "config" "dbpass" ] ''
      Please use `services.myNextcloud.config.dbpassFile' instead!
    '')
    (mkRemovedOptionModule [ "services" "myNextcloud" "nginx" "enable" ] ''
    ...
    '')
    (mkRemovedOptionModule [ "services" "myNextcloud" "disableImagemagick" ] ''
      Use services.myNextcloud.enableImagemagick instead.
    '')
  ];

  options.services.myNextcloud = {  # MODIFIED
    ...
    datadir = mkOption {
      type = types.str;
      default = config.services.myNextcloud.home;
      defaultText = literalExpression "config.services.myNextcloud.home";
      description = lib.mdDoc ''
        Data storage path of nextcloud.  Will be [](#opt-services.myNextcloud.home) by default.
    ...
    ## MOD
    truedatadir = mkOption {
      type = types.str;
      default = "${config.services.myNextcloud.home}/data";
      defaultText = literalExpression "${config.services.myNextcloud.home}/data";
      description = lib.mdDoc ''
        True data storage path of nextcloud.
      '';
      example = "/mnt/nextcloud-file";
    };
    ## MOD
    ...
    extraApps = mkOption {
      ...
      description = lib.mdDoc ''
        ...
        Using this will disable the appstore to prevent Nextcloud from updating these apps (see [](#opt-services.myNextcloud.appstoreEnable)).
      '';
    ...
    extraAppsEnable = mkOption {
      ...
      default = true;
      description = lib.mdDoc ''
        Automatically enable the apps in [](#opt-services.myNextcloud.extraApps) every time nextcloud starts.
    ...
    appstoreEnable = mkOption {
      ...
      description = lib.mdDoc ''
        ...
        Enabled by default unless there are packages in [](#opt-services.myNextcloud.extraApps).
        Set to true to force enable the store even if [](#opt-services.myNextcloud.extraApps) is used.
    ...
    logType = mkOption {
      ...
      description = lib.mdDoc ''
        ...
        systemd requires the php-systemd package to be added to services.myNextcloud.phpExtraExtensions.
    ...
    config = {
      ...
      extraTrustedDomains = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          `services.myNextcloud.hostname` here.
        '';
      };
    ...
    secretFile = mkOption {
      ...
      description = lib.mdDoc ''
        ...
        form as the [](#opt-services.myNextcloud.extraOptions) option), for example
  ...
  config = mkIf cfg.enable (mkMerge [
    { warnings = let
      ...
      in (optional (cfg.poolConfig != null) ''
          Using config.services.myNextcloud.poolConfig is deprecated and will become unsupported in a future release.
          Please migrate your configuration to config.services.myNextcloud.poolSettings.
        '')
        ...
        #++ (optional cfg.enableBrokenCiphersForSSE ''
          ...
          #rid of this warning by declaring
            #services.myNextcloud.enableBrokenCiphersForSSE = false;
      ...
      services.myNextcloud.package = with pkgs;
        mkDefault (
          if pkgs ? nextcloud
            then throw ''
              The `pkgs.nextcloud`-attribute has been removed. If it's supposed to be the default
              nextcloud defined in an overlay, please set `services.myNextcloud.package` to
              `pkgs.nextcloud`.
            ''
      ...
      services.myNextcloud.phpPackage =
    ...
    { assertions = [
      { assertion = cfg.database.createLocally -> cfg.config.dbtype == "mysql";
        message = ''services.myNextcloud.config.dbtype must be set to mysql if services.myNextcloud.database.createLocally is set to true.'';
      }
    ]; }
      ...
      systemd.services = {
        ...
        in {
          ...
          script = ''
            ...
            for dir in ${datadir}/config ${cfg.home}/store-apps ${cfg.home}/nix-apps; do
              if [ ! -e $dir ]; then
